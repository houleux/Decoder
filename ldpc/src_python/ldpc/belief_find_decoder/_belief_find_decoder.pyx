...
cdef class BeliefFindDecoder(BpDecoderBase):
    ...
    def decode(self,syndrome):
        ...
        # BP expects an LLR vector (vector[double]); construct it from channel probabilities
        # LLR_i = log((1 - p_i) / p_i), where p_i is the bit-flip probability
        ...
        for i in range(self.n):
            ...
            if p <= 0.0:
                ...
                p = 1.0 - 1e-12
            llr_vector[i] = np.log((1.0 - p) / p)

        # Run BP to populate internal decoding and log_prob_ratios
        self.bpd.decode(llr_vector)
