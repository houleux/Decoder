...
class LDPCDecoderEnv(gym.Env):
    """
    Gymnasium environment for LDPC decoding using cluster scheduling.
    
    Action space: Discrete(num_clusters) - select which cluster to schedule
    Observation space: LLR vector (Box)
    Reward: 1 if action is 0, 0 otherwise
    """
    ...
    def __init__(self, render_mode=None, H_path=None, num_clusters=6, max_iterations=30, snr_db=0):
        ...
        # Observation space: LLR vector
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(self.n_code,), dtype=np.float32
        )
    ...
    def _get_obs(self):
        """Return current LLR vector as observation."""
    ...
    def _get_info(self):
        ...
        return {
            ...
            "syndrome_weight": int(np.sum(np.abs(self.decoder.syndrome))) if hasattr(self.decoder, 'syndrome') else 0,
        }
    ...
    def step(self, action):
        ...
        info['decoded_correctly'] = np.array_equal(decoded_codeword[:self.n], self.message)
    ...
    def _render_frame(self):
        ...
        if self.render_mode == "human":
            ...
            if self.current_llr is not None:
                ...
                print(f"  Mean: {np.mean(self.current_llr):.4f}")
                print(f"  Std:  {np.std(self.current_llr):.4f}")
                print(f"  Min:  {np.min(self.current_llr):.4f}")
                print(f"  Max:  {np.max(self.current_llr):.4f}")
            info = self._get_info()
