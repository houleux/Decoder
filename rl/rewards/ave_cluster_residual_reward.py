...
class AveClusterResidualReward:
    """
    Reward = Average absolute residual (|LLR_after - LLR_before|) 
    across all Variable Nodes in the cluster's neighborhood.
    
    This is a channel-agnostic reward signal (doesn't assume all-zero codeword).
    ...
    def compute(self, llr_before: np.ndarray, llr_post: np.ndarray) -> float:
        ...
        Returns:
            Average residual (float >= 0).
        """
        if len(self.neighborhood) == 0:
            return 0.0
        return float(np.mean(np.abs(llr_post[self.neighborhood] - llr_before[self.neighborhood])))
