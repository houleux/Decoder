...
class LocalAveTanhLLRState:
    """
    State encoder: Average tanh(LLR) of all VNs in the cluster's neighborhood.
    ...
    def encode(self, llr_post: np.ndarray) -> tuple | float:
        ...
        Returns:
            Tuple of one int (if discretize=True) or Float representing the average tanh(LLR).
        """
        if len(self.neighborhood) == 0:
            return (discretize_value(0.0, -1.0, 1.0, self.n_bins),) if self.discretize else 0.0
            
        ave_tanh_llr = float(np.mean(np.tanh(llr_post[self.neighborhood])))
        
        if self.discretize:
            return (discretize_value(ave_tanh_llr, -1.0, 1.0, self.n_bins),)
        else:
            return ave_tanh_llr
