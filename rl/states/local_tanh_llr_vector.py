...
class LocalTanhLLRVectorState:
    """
    State encoder: Vector of tanh(LLRs) of all VNs in the cluster's neighborhood.
    ...
    def encode(self, llr_post: np.ndarray) -> tuple:
        ...
        """
        if self.discretize:
            return tuple(discretize_value(float(np.tanh(x)), -1.0, 1.0, self.n_bins) for x in llr_post[self.neighborhood])
        else:
            return tuple(float(np.tanh(x)) for x in llr_post[self.neighborhood])
