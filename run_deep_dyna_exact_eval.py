#!/usr/bin/env python3
"""
Evaluate DeepDynaExact (Factored MDP with Deep Q-Network).
State for each action: integer in [0, 63].
Network evaluates Q(local_state_int, action_int).
"""
import time
import numpy as np
...
# ── Factored Q-Network Exact ──────────────────────────────────────────────────

class FactoredQNetworkExact(nn.Module):
    def __init__(self, num_actions: int, hidden_dim: int):
        super().__init__()
        self.num_actions = num_actions
        self.state_emb = nn.Embedding(64, hidden_dim)
    ...
    def forward(self, local_state_int: torch.Tensor, action_int: torch.Tensor) -> torch.Tensor:
        # local_state_int: (B,) int64
        # action_int: (B,) int64
        s_emb = self.state_emb(local_state_int)
        a_emb = self.action_emb(action_int)
        x = torch.cat([s_emb, a_emb], dim=-1)
        return self.net(x)

# ── Factored Trainer Exact ───────────────────────────────────────────────────

class DeepDynaExactTrainer(Trainer):
    def __init__(self, h_csr, config, beta_discount, l_max, device="cpu"):
        ...
        self.online_net = FactoredQNetworkExact(self.num_actions, config.hidden_dim).to(self.device)
        self.target_net = FactoredQNetworkExact(self.num_actions, config.hidden_dim).to(self.device)
        self.target_net.load_state_dict(self.online_net.state_dict())
        self.target_net.eval()
        
        self.optimizer = torch.optim.Adam(self.online_net.parameters(), lr=config.learning_rate)
        # ReplayBuffer stores floats, but we will pass integers in [0, 63]
        ...
        self._action_tensor = torch.arange(self.num_actions, device=self.device, dtype=torch.int64)

    def _get_global_discrete_state(self, llr_post: np.ndarray) -> np.ndarray:
        state = np.zeros(self.num_actions, dtype=np.float32)
        for a in range(self.num_actions):
            state[a] = float(_state_from_llr_subset(llr_post, self.map.cluster_neighbors[a]))
        return state
    ...
    def _choose_action(self, global_state: np.ndarray, rng: np.random.Generator, training: bool) -> int:
        ...
        with torch.no_grad():
            st = torch.as_tensor(global_state, dtype=torch.int64, device=self.device) # (num_actions,)
    ...
    def _planning_step(self, rng: np.random.Generator) -> float:
        ...
        st = torch.as_tensor(states, dtype=torch.int64, device=self.device) # (B, num_actions)
        ...
        s_taken = st[torch.arange(bs), at] # (B,)
        q_vals = self.online_net(s_taken, at).squeeze(1) # (B,)
        
        ns = torch.as_tensor(next_states, dtype=torch.int64, device=self.device) # (B, num_actions)
        ns_flat = ns.view(-1) # (B * num_actions)
...
# ── Factored Decoder Exact ───────────────────────────────────────────────────

class DeepDynaExactDecoder:
    ...
    def decode(self, llr_channel, i_max):
        ...
        for i in range(i_max):
            state = np.zeros(self.num_actions, dtype=np.int64)
            for a in range(self.num_actions):
                state[a] = _state_from_llr_subset(llr_post, self.map.cluster_neighbors[a])
            
            with torch.no_grad():
                st = torch.as_tensor(state, dtype=torch.int64, device=self.device)
...
def evaluate_exact_method(decoder, snr_db, code_rate, i_max, target_errs, max_frames, rng):
    from RELDEC.algorithms.reldec_core import MethodStats, bpsk_awgn_llr, DecodeResult
    stats = MethodStats(method="deep_dyna_exact", n=decoder.n)
...
def _exact_worker(args):
    ...
    net = FactoredQNetworkExact(num_act, h_dim)
    net.load_state_dict(state_dict)
    decoder = DeepDynaExactDecoder(h_csr, net, c_size, "cpu")
    rng = np.random.default_rng(seed)
    return evaluate_exact_method(decoder, snr_db, cr, i_max, errs, frames, rng)
...
def main():
    ...
    print(f"\n--- Training DeepDynaExact ({episodes} eps) ---")
    f_config = DeepDynaConfig(
        policy_label="exact", cluster_size=1, n_planning_steps=10,
    ...
    f_trainer = DeepDynaExactTrainer(h, f_config, 0.9, 50, "cpu")
    t0 = time.time()
    f_prog = f_trainer.train(cfg)
    t_exact = time.time() - t0
    print(f"  done in {t_exact:.2f}s  |  total_reward={f_prog.reward_sum:.1f}")
    ...
    with ProcessPoolExecutor(max_workers=workers) as pool:
        for fut in as_completed({pool.submit(_exact_worker, a): i for i, a in enumerate(worker_args)}):
            partials.append(fut.result())
    f_stats = merge_method_stats(partials)
    f_stats.method = "deep_dyna_exact"
    e_exact = time.time() - t0

    print(f"\n{'Method':20s} {'FER':>12s} {'BER':>12s} {'AvgMsgs':>10s} {'EvalTime':>10s} {'TrainTime':>10s}")
    print("-" * 80)
    for name, stats, etime, ttime in [("dyna", d_stats, e_dyna, t_dyna), ("deep_dyna_exact", f_stats, e_exact, t_exact)]:
